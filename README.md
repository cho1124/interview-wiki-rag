...
사용자 → Next.js (Cloudflare Pages)
  → /api/chat
    ...
    → 관련 섹션 추출 (토큰 절감)
    → Cloudflare Workers AI (Qwen3-30B-A3B MoE)
    → 스트리밍 응답
```
...
| AI 모델 | Cloudflare Workers AI (Qwen3-30B-A3B MoE, 3B 활성) | 무료 |
...
### 5차: 모델 벤치마크 및 교체 (7개 모델 비교)

4차에서 발견된 "Llama 8B 인용 미준수" 문제를 해결하기 위해 7개 모델을 벤치마크.

| 모델 | 속도 | 인용 준수 | 한국어 | 판정 |
|------|------|:---------:|:------:|------|
| **Qwen3-30B-A3B** | **3.7s** | **A+** | **A+** | **채택** |
| Gemma 4 26B-A4B | 6.2s | A+ | A+ | 2순위 |
| Kimi K2 | 10.1s | A+ | A | 3순위 |
| Nemotron 120B | 40.3s | A | A | 느림 |
| DeepSeek V3.2 | 36.5s | A | A | 느림 |
| Nemotron Nano 30B | 3.4s | F (0개) | A | 인용 불가 |
| GLM-5 | timeout | - | - | 사용 불가 |

Qwen3-30B-A3B는 MoE 구조로 3B만 활성화되어 무료 티어에서 효율적이면서, CJK 언어에 특화되어 한국어 면접 RAG에 최적.
